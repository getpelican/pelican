...
def configure_settings(settings):
    ...
    if (
        ...
        and settings.get("WITH_FUTURE_DATES", False)
    ):
        logger.warning(
    ...
    pagination_rules = [
        ...
        for r in settings.get(
            "PAGINATION_PATTERNS", DEFAULT_CONFIG["PAGINATION_PATTERNS"],
        )
    ]
    ...
    for k, v in overrides.items():
        ...
        if setting_type not in types_to_cast:
            ...
            try:
                ...
                logger.debug(
                    "ValueError for %s override with %s, try to " "load as json", k, v
                )
                coerced[k] = json.loads(v)
    return coerced
