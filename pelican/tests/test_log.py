...
class TestLog(unittest.TestCase):
    ...
    def test_log_formatter(self):
        ...
        with self.reset_logger():
            ...
            self.assertEqual(counter("Log test", logging.WARNING), 1)
        ...
        with self.reset_logger():
            ...
            # Log
            # | test
            self.assertEqual(counter("Log", logging.WARNING), 1)
            self.assertEqual(counter(" | test", logging.WARNING), 1)
        ...
        with self.reset_logger():
            ...
            # Log test1
            # | test2
            self.assertEqual(counter("Log test1", logging.WARNING), 1)
            self.assertEqual(counter(" | test2", logging.WARNING), 1)

        with self.reset_logger():
            # log single list
            self.logger.warning("Log %s", ["foo", "bar"])
            self.assertEqual(counter(r"Log \['foo', 'bar'\]", logging.WARNING), 1)
