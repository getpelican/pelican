...
needed by Pelican.
    ...
    try:
        with open(
            os.path.join(CONF["basedir"], "pelicanconf.py"), "w", encoding="utf-8"
        ) as fd:
            conf_python = dict()
    ...
    try:
        with open(
            os.path.join(CONF["basedir"], "publishconf.py"), "w", encoding="utf-8"
        ) as fd:
    ...
    if automation:
        try:
            with open(
                os.path.join(CONF["basedir"], "tasks.py"), "w", encoding="utf-8"
            ) as fd:
        ...
        try:
            with open(
                os.path.join(CONF["basedir"], "Makefile"), "w", encoding="utf-8"
            ) as fd:
                py_v = "python3"
