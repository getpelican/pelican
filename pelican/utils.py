...
class FileSystemWatcher:
    def __init__(self, settings_file, reader_class, settings=None):
        self.watchers = {"settings": FileSystemWatcher.file_watcher(settings_file)}
    ...
    def update_watchers(self, settings):
        ...
        if extensions_changed or content_changed or ignore_changed:
            self.add_watcher(
                "content", new_content_path, new_extensions, new_ignore_files
            )

        # Refresh theme watcher if related settings changed
        if theme_changed or ignore_changed:
            self.add_watcher("theme", new_theme_path, [""], new_ignore_files)

        # Watch STATIC_PATHS
        old_static_watchers = set(
            key for key in self.watchers if key.startswith("[static]")
        )
        ...
        for path in settings.get("STATIC_PATHS", []):
            ...
            if ignore_changed or (key not in self.watchers):
                self.add_watcher(
                    key, os.path.join(new_content_path, path), [""], new_ignore_files
                )
        ...
        def file_times(path):
            ...
            for root, dirs, files in os.walk(path, followlinks=True):
                ...
                for f in files:
                    ...
                    file_ignored = any(fnmatch.fnmatch(f, ignore) for ignore in ignores)
