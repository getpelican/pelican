...
def process_translations(content_list, order_by=None):
    ...
    if order_by:
        ...
        content_list.sort(key=lambda x:os.path.basename(x.source_path))

    if order_by:
        if hasattr(order_by, '__call__'):
            grouped_by_slugs = groupby(content_list, order_by)
        else:
            grouped_by_slugs = groupby(content_list, attrgetter(order_by))
    else:
        grouped_by_slugs = groupby(content_list,
                lambda x:os.path.basename(x.source_path))
